...
Shuffle the specialist cards back into the deck, draw and place new cards, laying one face up on each space on the Specialist Staff track of the main board.

#### 2\. Place Staff Tokens

Each turn a player has up to three staff tokens per turn they can place on the game board, the actions triggered by placing these staff happen in phase 4, but for ease of reading, the manual will summarise the resulting actions here.
...
**Research & Development** R&D Resources are used to allocate to draw a product card from the product area of the main board.

The specialist staff are your main method for turning money into these resources, so use them wisely.

Different specialists help you generate different resources, and there are two different levels of specialists, experts and geniuses.

##### Draw a Product

Products are the main focus of Chip Shop and players main aim should be to build them.

When taking this action, players decide from which slot on the products track of the main board they wish to draw a card from and pay the R&D cost when they have decided on the product to build. These represent sub-eras of the 80s, and one square is roughly equal to 2 years, there is also a square for accessories.
...
When taking this action, players place their staff token(s) in the order they are placed, (i.e. the first player at the top, the last at the bottom), there is no limit as to how many staff tokens can be placed here, but the resulting actions must happen in order.

When it comes to the action phase, in token order players search teh deck for a product they would liek, places it in their factory, shuffles the deck and places it back.
...
You may also draw products from the product discard pile to the left of the board, these cost no R&D but lose loyalty and longevity (**HOW MUCH?**).
...
2\. **Ensure you have at least one resource on your player mat or on a product. If not, you are declared bankrupt.**
2. Determine turn order for the next turn. This is in order of longevity score, from lowest to highest. In the event of a tie for longevity score, then highest loyalty score wins, if this ties, then use the turn order from last turn.
